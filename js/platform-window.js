...
const wrappedCurrentLayout = fin.Platform.Layout.getCurrentSync();

fin.me.on('layout-ready', async payload => {
    if (payload.name !== fin.me.identity.name) {
        return;
    }
    const { settings } = await wrappedCurrentLayout.getConfig();
...
class LeftMenu extends HTMLElement {
    ...
    async saveWindowLayout() {
        const winLayoutConfig = await wrappedCurrentLayout.getConfig();
    ...
    async restoreWindowLayout() {
        ...
        if (storedWinLayout) {
            return wrappedCurrentLayout.replace(JSON.parse(storedWinLayout));
        } else {
    ...
    async toGrid() {
        await wrappedCurrentLayout.applyPreset({
            presetType: 'grid'
        });
    }
...
class TitleBar extends HTMLElement {
    ...
    async toggleLockedLayout () {
        const oldLayout = await wrappedCurrentLayout.getConfig();
        const { settings, dimensions } = oldLayout;
        if(settings.hasHeaders && settings.reorderEnabled) {
            wrappedCurrentLayout.replace({
                ...oldLayout,
                settings: {
                    ...settings,
                    hasHeaders: false,
