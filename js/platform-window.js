...
class LeftMenu extends HTMLElement {
    ...
    async render() {
        ...
        <div class="left-menu">
            <ul>
                <li><button @click=${() =>
                    this.createChart().catch(
                        console.error
                    )}>New Chart</button></li>
                <li><button @click=${() =>
                    this.newDefaultWindow().catch(
                        console.error
                    )}>New Chart Window</button></li>
                <li><button @click=${() =>
                    this.nonLayoutWindow().catch(
                        console.error
                    )}>New Window</button></li>
                <li><button class="new" @click=${() =>
                    this.nativeSingleWindow().catch(
                        console.error
                    )}>Native Single Window</button></li>
                <li><button class="new" @click=${() =>
                    this.nativeSingleWpfWindow().catch(
                        console.error
                    )}>Native Single Wpf Window</button></li>
                <li><button class="new" @click=${() =>
                    this.nativeMultiWindow().catch(
                        console.error
                    )}>Native Multi Window</button></li>
                <li><button class="secret" @click=${() =>
                    this.nativeView().catch(
                        console.error
                    )}>Bonus Round</button></li>
                <li><button class="secret" @click=${() =>
                    this.nativeViewWindow().catch(
                        console.error
                    )}>Bonus Window</button></li>
                <li><button @click=${() =>
                    this.saveSnapshot().catch(
                        console.error
                    )}>Save Platform Snapshot</button></li>
                <li><button @click=${() =>
                    this.restoreSnapshot().catch(
                        console.error
                    )}>Restore Platform Snapshot</button></li>
                <li><button @click=${() =>
                    this.applySnapshot().catch(
                        console.error
                    )}>Apply Platform Snapshot</button></li>
            <ul>
        </div>`;
    ...
    async saveWindowLayout() {
        ...
        localStorage.setItem(
            fin.me.identity.name,
            JSON.stringify(winLayoutConfig)
        );
    }
    ...
    async restoreWindowLayout() {
        ...
        if (storedWinLayout) {
            return fin.Platform.Layout.getCurrentSync().replace(
                JSON.parse(storedWinLayout)
            );
        } else {
            ...
            windows: [
                {
                    ...
                    defaultLeft: 200,
                    defaultTop: 200,
                    className: "OpenFinPlatformApp",
                    mainWindow: true,
                    startInfo: {
                        alias: "openfin-platform-wpf-app",
                        arguments: "--platform",
                    },
                },
            ],
